...
mkdir -p /tmp/outputfiles/$(basename ${file::-2})
cd /tmp/outputfiles/ # $(basename ${file::-2})
...
java -Xmx90G -jar $msfraggerPath $fraggerParamsPath /tmp/timstoffiles/*.d

#Now we want to move the pepxml files from file directory, to output directory
mv /tmp/timstoffiles/$(basename ${file::-2}).pepXML /tmp/outputfiles/$(basename ${file::-2})
...
cp -r /tmp/outputfiles/$(basename ${file::-2}) /mnt/Path-Steen/analysis/outputfiles/
